...
export const validateSongId = async (songId: string): Promise<boolean> => {
    ...
    try {
      const response = await axios.get(`http://host.docker.internal:8080/api/song/${songId}`);
...
export const getList = async (songId: string, size: number, from: number): Promise<StreamDetailsDto[]> => {
...
export const getStatsForSongs = async (ids: string[]): Promise<Record<string, number>> => {
